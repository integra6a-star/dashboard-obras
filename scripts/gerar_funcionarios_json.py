...
def normalize_sexo(sexo_txt: str) -> str:
    s = norm_header(sexo_txt)
    if not s:
        return ""
    if s in ("m", "masc", "masculino", "homem"):
        return "Masculino"
    if s in ("f", "fem", "feminino", "mulher"):
        return "Feminino"
    if s.startswith("m"):
        return "Masculino"
    if s.startswith("f"):
        return "Feminino"
    return sexo_txt.strip()
...
def read_rows(values_sh, formulas_sh):
    ...
    c_resp   = col(hmap, "Responsavel_Direto", "Responsável Direto", "Responsavel Direto", "responsavel_direto")
    ...
    c_sexo   = col(hmap, "Sexo", "Gênero", "Genero")
    ...
    c_resp   = idx_or_fallback(c_resp,   "ResponsavelDireto")
    ...
    for r in range(header_row + 1, max_row + 1):
        ...
        sexo_val = safe_cell_value(values_sh, formulas_sh, r, c_sexo) if c_sexo else None
        sexo = normalize_sexo(to_text(sexo_val))
        if nome == "":
            continue
        ...
        responsavel_direto = get_text(c_resp)
...
def read_planilha_totais(values_sh):
    header_row = detect_header_row(values_sh)
    hmap = build_header_map(values_sh, header_row)

    def money_from_header(*names):
        c = col(hmap, *names)
        if not c:
            return 0.0
        return round(to_float(values_sh.cell(row=1, column=c).value), 2)

    return {
        "total_salarios": money_from_header("Custo TOTAL MO", "Custo Total MO"),
        "salario_base": money_from_header("Salario", "Salário"),
        "total_he_dsr": money_from_header("Custo HE c/ DSR", "Custo HE c DSR"),
        "total_vr": money_from_header("vale_refeicao", "Vale Refeicao", "Vale Refeição"),
        "total_valor_veiculo": money_from_header("valor_veiculo", "Valor Veiculo", "Valor Veículo"),
        "total_combustivel": money_from_header("combustivel", "Combustivel", "Combustível"),
        "total_valor_mensal": money_from_header("valor_mensal", "Valor Mensal"),
        "custo_pessoal": money_from_header("custo_pessoal", "Custo Pessoal"),
        "total_frota": money_from_header("custo_frota", "Custo Frota"),
        "custo_mensal_total": money_from_header("custo_total", "Custo Total"),
    }
...
def main():
    ...
    for r in registros:
        ...
        responsavel = r.get("responsavel_direto") or "Sem Responsável"
        ...
        por_responsavel_direto[responsavel] = por_responsavel_direto.get(responsavel, 0.0) + r["custo_total"]
    ...
    out = {
        ...
        "por_responsavel_direto": {k: round(v, 2) for k, v in sorted(por_responsavel_direto.items())},
