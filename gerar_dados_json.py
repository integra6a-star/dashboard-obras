...
# --- Caminhos robustos (independente de onde rodar) ---
SCRIPT_DIR = Path(__file__).resolve().parent
ROOT_DIR = SCRIPT_DIR.parent  # pasta dashboard-obras
...
CANDIDATOS_EXCEL = [
    ...
    SCRIPT_DIR / ".." / "docs" / EXCEL_NAME,
]

ARQ_EXCEL = next((p.resolve() for p in CANDIDATOS_EXCEL if Path(p).resolve().exists()), None)
...
def main():
    if ARQ_EXCEL is None:
        raise FileNotFoundError(
            f"Arquivo '{EXCEL_NAME}' não encontrado.\n"
            f"Coloque ele na raiz ({ROOT_DIR}) ou em ({ROOT_DIR / 'docs'})."
        )
